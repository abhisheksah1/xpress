...
/** Ensures heading/link colors contrast with the footer background (fixes legacy dark-theme colors). */
...
export function resolveFooterLayout(footerNav) {
  if (footerNav?.footerLayout?.linkColumns?.length) {
    return {
      ...DEFAULT_FOOTER_LAYOUT,
      ...footerNav.footerLayout,
      linkColumns: footerNav.footerLayout.linkColumns,
      infoColumns: footerNav.footerLayout.infoColumns || DEFAULT_FOOTER_LAYOUT.infoColumns,
    };
  }

  const legacyItems = (footerNav?.items || []).filter((i) => i.isActive !== false);
  if (legacyItems.length) {
    return {
      ...
      linkColumns: [
        {
          title: 'Quick Links',
          items: legacyItems.map((i) => ({ label: i.label, link: i.link, openInNewTab: i.openInNewTab })),
        },
        ...DEFAULT_FOOTER_LAYOUT.linkColumns.slice(1),
      ],
    };
  }
